...
export function PostDetailDialog({ post, canManage, open, onClose }: { post: BoardPost; canManage: boolean; open: boolean; onClose: () => void }) {
  ...
  useEffect(() => { setCaption(post.caption); setSectionId(post.sectionId); }, [post]);
    ...
    <section className="dialog post-detail-dialog" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className="comments-panel"><h3>Comments <span>{postComments.length}</span></h3>
        {postComments.map((comment) => <article className="comment-item" key={comment.id}>
          <span className="avatar avatar-small">{comment.authorInitials}</span><div><strong>{comment.authorName}</strong><p>{comment.text}</p>{comment.mainImageUrl && <img src={comment.thumbImageUrl || comment.mainImageUrl} alt={comment.text || "Comment attachment"} />}
