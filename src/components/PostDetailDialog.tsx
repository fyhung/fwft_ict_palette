...
export function PostDetailDialog({ post, canManage, startEditing = false, open, onClose, postPosition = 1, postCount = 1, onPreviousPost, onNextPost }: { post: BoardPost; canManage: boolean; startEditing?: boolean; open: boolean; onClose: () => void; postPosition?: number; postCount?: number; onPreviousPost?: () => void; onNextPost?: () => void }) {
  ...
  const media = useMemo<ViewerMediaItem[]>(() => {
    const items: ViewerMediaItem[] = [];
    const postImage = post.mainImageUrl || post.imageUrl;
    if (postImage) items.push({ id: `post-${post.id}`, src: postImage, alt: post.caption, label: "Post photo" });
    postComments.forEach((comment) => {
      if (comment.mainImageUrl) items.push({ id: `comment-${comment.id}`, src: comment.mainImageUrl, alt: comment.text || "Comment photo", label: `Comment photo by ${comment.authorName}` });
    });
    return items;
  }, [post, postComments]);
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      ...
      if (galleryIndex !== undefined) setGalleryIndex(undefined); else onClose();
    };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [galleryIndex, onClose, open]);
  ...
  return <><div className="dialog-backdrop" onMouseDown={onClose}>
    <section className="dialog post-detail-dialog" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}>
      ...
      <button className="media-open-button post-main-view" onClick={() => setGalleryIndex(0)}>
  ...
  </div>{galleryIndex !== undefined && <FullscreenMediaViewer media={media} initialIndex={galleryIndex} postTitle={post.caption} postPosition={postPosition} postCount={postCount} onClose={() => setGalleryIndex(undefined)} onPreviousPost={onPreviousPost} onNextPost={onNextPost} />}</>;
}
